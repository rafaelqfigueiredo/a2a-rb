...
require "a2a/oauth_flow/device_code"

RSpec.describe A2A::OAuthFlow::DeviceCode do
  describe "#initialize" do
    it "sets required attributes" do
      ...
      expect(flow.device_authorization_url).to eq "https://example.com/device"
  ...
  describe ".from_h" do
    ...
    it "raises KeyError when deviceAuthorizationUrl is missing" do
      expect {
    ...
    it "raises KeyError when tokenUrl is missing" do
      expect {
        described_class.from_h("deviceAuthorizationUrl" => "https://example.com/device", "scopes" => {})
      }.to raise_error(KeyError)
    end

    it "raises KeyError when scopes is missing" do
      expect {
        described_class.from_h("deviceAuthorizationUrl" => "https://example.com/device", "tokenUrl" => "https://example.com/token")
      }.to raise_error(KeyError)
    end
  end
end
